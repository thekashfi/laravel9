                    ...
                    <div class="pricing-box tns-item tns-slide-cloned" aria-hidden="true" tabindex="-1">
                        <div class="single-pricing">
                            <div class="accordion" id="accordionExample">
                                ...
                                @if($item['category']->name!="هدیه")
                                <div >
</br>
                                    <h2 class="accordion-header" id="heading{{$key}}" style="background-color:green; border-radius:6px;">
                                        <button class="accordion-button collapsed" type="button"
                                            data-bs-toggle="collapse" data-bs-target="#collapse{{$key}}"
                                            aria-expanded="false" aria-controls="collapse{{$key}}" style="color:black; background-color:green; border-radius:6px;">
                                            تعداد {{ $item['category']->name }}‌ها: {{ $item['count'] }} عدد
                                        </button>
                                    </h2>
                                    <div id="collapse{{$key}}" class="accordion-collapse collapse"
                                        aria-labelledby="heading{{$key}}" data-bs-parent="#accordionExample">
                                        <div class="accordion-body" style="background-color:lightgreen; border-radius:6px;">
                                            <ul class="content" >
                                                @foreach($item['contracts'] as $contract)
                                                <li>
                                                    @if( $contract->is_active )
                                                    <a href="{{ route('contract' , $contract->slug) }}" target="_blank">
                                                        {{ $contract->name }} @if( $item['category']->hidden )(به ارزش:
                                                        {{ number_format($contract->old_price, null, '.', ',') }}<span
                                                            class="toman">تومان</span>)@endif
                                                    </a>
                                                    @else
                                                    {{ $contract->name }} @if( $item['category']->hidden )(به ارزش:
                                                    {{ number_format($contract->old_price, null, '.', ',') }}<span
                                                ...
                                                <li>
                                                    ...
                                                    <a href="{{ route('file' , $file->slug) }}" target="_blank">
                                                        {{ $file->name }} @if( $item['category']->hidden )(به ارزش:
                                                        {{ number_format($file->old_price, null, '.', ',') }}<span
                                                            class="toman">تومان</span>)@endif
                                                    </a>
                                                    @else
                                                    {{ $file->name }} @if( $item['category']->hidden )(به ارزش:
                            ...
                            <div style='display: flex; flex-direction: row;'>
                                ...
                                <lottie-player src="https://assets7.lottiefiles.com/packages/lf20_wrbylkbm.json"
                                    background="transparent" speed="1" style="width: 90px; height: 90px;" loop autoplay>
                                </lottie-player>

                                <h2 style="vertical-align: bottom; color:#957f47;">
                                    </br>
                                    هدایای ویژه
                                </h2>
                                <div>
                                    ...
                                    <lottie-player src="https://assets2.lottiefiles.com/packages/lf20_7imyxpi7.json"
