        ...
        <form method="post" enctype="multipart/form-data" action="{{ route($package->id === null ? 'admin.package.store' : 'admin.package.update', $package->id) }}">
            ...
            <div class="row">
                    ...
                    <select name="category_id[]" multiple class="form-select resize-vertical"  x-model="category_id" id="category_id">
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ in_array($category->id , old('category_id', optional($package->categories)->pluck('id')->toArray() ) ) ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 pb-3">
                    ...
                    <input dir="ltr" name="order" type="number" class="form-control" id="order" placeholder="" value="{{ old('order', $package->order) }}">
                </div>
